...
ton: $(TON_C_FILES) $(TON_H_FILES) wordlist/generatedwordlist.c
	$(CC) $(CFLAGS) -Wpedantic -DUNIX=1 -DTON_GIT_COMMIT_HASH="\"$(GIT_COMMIT_HASH)\"" -o ton $(TON_C_FILES) wordlist/generatedwordlist.c $(LDFLAGS)
...
ton.exe: $(TON_C_FILES) $(TON_H_FILES) wordlist/generatedwordlist.c
ifndef MINGW_OPENSSL_ROOT
	...
	$(MINGW_CC) $(CFLAGS) -DWINDOWS=1 -DTON_GIT_COMMIT_HASH="\"$(GIT_COMMIT_HASH)\"" -static $(MINGW_OPENSSL_INCLUDE) $(MINGW_CUNIT_INCLUDE) -o ton.exe $(TON_C_FILES) wordlist/generatedwordlist.c -L$(MINGW_OPENSSL_ROOT) $(MINGW_LINK_CUNIT_PATH) $(LDFLAGS) -liphlpapi -lws2_32
endif

# Automatically-generated C file: this contains the list of words that can
# appear in a passphrase, as a C array of strings. We generate this file from
# wordlist/wordlist.txt using a Python script.
