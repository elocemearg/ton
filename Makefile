...
ton.exe: $(TON_C_FILES) $(TON_H_FILES) src/wordlist.txt
ifndef MINGW_OPENSSL_ROOT
	...
	$(MINGW_CC) $(CFLAGS) -DWINDOWS=1 -DTON_GIT_COMMIT_HASH="\"$(GIT_COMMIT_HASH)\"" -static $(MINGW_OPENSSL_INCLUDE) $(MINGW_CUNIT_INCLUDE) -o ton.exe $(TON_C_FILES) -Wl,--format=binary -Wl,src/wordlist.txt -Wl,--format=default -L$(MINGW_OPENSSL_ROOT) $(MINGW_LINK_CUNIT_PATH) $(LDFLAGS) -liphlpapi -lws2_32
endif
