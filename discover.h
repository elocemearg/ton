#ifndef _TONDISCOVER_H
#define _TONDISCOVER_H
...
/* Callback function which tondlctx_receive will call to say it's successfully
 * set things up and is now waiting for announcements. */
typedef void (*tondl_listening_cb)(void *);

/* Callback to tell the caller when we receive an announcement datagram,
 * valid or not */
typedef void (*tondl_received_announcement_cb)(void *, const struct sockaddr *, socklen_t, int valid, int invitation_port);
...
typedef int (*tonda_sent_announcement_cb)(void *cookie,
...
struct ton_discover_options {
     ...
     * NULL we use the default, TON_MULTICAST_GROUP_IPV4 and
     * TON_MULTICAST_GROUP_IPV6. The other host must use the same address. */
    ...
    /* address_families: whether to listen for announcement datagrams on IPv4,
       IPv6, or both. Set it to TON_IPV4_ONLY, TON_IPV6_ONLY, or TON_IP_BOTH. */
    int address_families;

    /* announcement_types: for ton_discover_and_accept(), whether to send out
     * the announcement messages using broadcast, multicast, or both. Set it to
     * TON_ANNOUNCE_BROADCAST, TON_ANNOUNCE_MULTICAST or TON_ANNOUNCE_BOTH. */
    int announcement_types;

    /* The UDP port on which to send or receive announcement datagrams. The
     * default is TON_DEFAULT_DISCOVER_PORT, and both ends must use the same. */
    int discover_port;
    ...
    /* Timeout, in milliseconds, for ton_discover_and_connect() to receive a
     * valid announcement datagram and establish a session to the other host.
     * ton_discover_and_connect() will fail if a session is not established
     * in that time. The default is 0, which disables the timeout. */
    int discover_connect_timeout_ms;
};

/* ton discovery listen context */
struct tondlctx {
...
/* ton discovery announce context */
struct tondactx {
    ...
    struct ton_netif *broadcast_ifs;
    ...
    struct ton_netif *multicast_ifs;
 ...
 * tondactx_set_invitation_port(). If the invitation port for a particular
 ...
 * ton_discover_and_connect().)
 *
 * Return 0 on success or nonzero on failure. */
int
tondactx_announce(struct tondactx *ctx, struct ton_discover_options *opts);

/* Destroy a previously-initialise ton discover-announce context and free any
 * resources associated with it. */
void
tondactx_destroy(struct tondactx *ctx);

/* Initialise a ton discover-listen context with the passphrase and options
 * in opts. This is called by the other endpoint to the one that called
 * tondactx_init(). It does not actually receive announcement datagrams: you
 * need to call tondlctx_receive(). */
int
 ...
 * interfaces.
 *
 * ctx must have been initialised with tondlctx_init().
 *
 * We hope to receive a datagram which looks like a ton announcement datagram
 ...
 * created upon the initialisation of the tondlctx, are in the given fd_set.
 *
 * If this returns 1, then there is something to receive on at least one of
 * our receiving sockets, and a call to tondlctx_receive() will not block.
 *
 * If this returns 0, then there is nothing to receive. */
int
tondlctx_fdset_contains_receivers(struct tondlctx *ctx, fd_set *set);

/* Destroy a previously-initialised ton discover-listen context and free any
 * resources associated with it. */
void
tondlctx_destroy(struct tondlctx *ctx);


/* Initialise a ton discover context, suitable for setting options in and
 * passing to ton_discover_and_connect() or ton_discover_and_accept().
 ...
 *
 * The default is TON_MULTICAST_GROUP_IPV4. */
int
 ...
 * begin with "ff".
 *
 * The default is TON_MULTICAST_GROUP_IPV6. */
int
 ...
 * accept or make a TCP connection. value must be TON_IPV4_ONLY, TON_IPV6_ONLY
 * or TON_IP_BOTH.
 *
 * The default is TON_IP_BOTH.
 */
void
ton_discover_set_address_families(struct ton_discover_options *opts, int value);

/* Set which announcement strategies we should use to send announcements for
 * the discovery stage.
 * Valid values are:
 * TON_ANNOUNCE_BROADCAST_ONLY: announce on broadcast addresses only.
 * TON_ANNOUNCE_MULTICAST_ONLY: announce on multicast addresses only.
 * TON_ANNOUNCE_BOTH: announce on both broadcast and multicast addresses.
 *
 * The default is TON_ANNOUNCE_BOTH.
 */
void
ton_discover_set_announcement_types(struct ton_discover_options *opts, int value);

/* Set the UDP port on which we send, or expect to receive, announcement
 * datagrams for the discovery stage. The default is TON_DEFAULT_DISCOVER_PORT.
 */
void
 ...
 * is TON_DEFAULT_LISTEN_PORT. If this is 0, we listen on any arbitrary port. */
void
ton_discover_set_listen_port(struct ton_discover_options *opts, int port);

/* Set verbose to 0 or 1 for frugal or profligate use of ton_error().
 * The default is 0. */
void
ton_discover_set_verbose(struct ton_discover_options *opts, int verbose);

/* Set the callback to be called when ton_discover_and_connect() starts
...
/* Set the callback to be called when ton_discover_and_connect() receives an
...
/* Set the callback to be called when ton_discover_and_accept() sends an
 ...
 * definitiopn of tonda_sent_announcement_cb at the top of this file. */
void
...
/* Set the maximum number of announcements ton_discover_and_accept() should
...
/* Set a timeout, in milliseconds, on ton_discover_and_connect(). If it takes
 * longer than this to receive a valid announcement datagram and connect to
 * the other host, ton_discover_and_connect() fails. The default is 0, which
 ...
 * handshook with the other end and created a ton_session. Nonzero on failure.
 */
int
ton_discover_and_connect(struct ton_discover_options *opts,
        struct ton_session *new_sess);
 ...
 *
 * This is the "other side" to ton_discover_and_connect().
 *
 * *new_sess: set to a new ton_session object if we received a connection on
 ...
 *   caller's responsibility to destroy this session (ton_session_destroy())
 ...
 */
int
ton_discover_and_accept(struct ton_discover_options *opts,
        struct ton_session *new_sess);

/* Free any resources associated with the ton_discover_options created with
 * ton_discover_options_init(). */
void
